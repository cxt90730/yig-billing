...
val df2 = sql("select m.ownerid,m.storageclass, sum(mp.size) from multipartpart mp left join multiparts m on m.bucketname = mp.bucketname group by m.ownerid,m.storageclass")
