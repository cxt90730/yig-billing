...
val df4 = sql("select ownerid,storageclass,sum((size + 65536 - 1) / 65536 * 65536) as usage from objects where storageclass=3 group by ownerid,storageclass")
val df5 = sql("select m.ownerid,m.storageclass, sum((mp.size + 65536 - 1) / 65536 * 65536) as usage from multipartpart mp left join multiparts m on m.bucketname = mp.bucketname and m.uploadtime=mp.uploadtime group by m.ownerid,m.storageclass").where("storageclass=3")
