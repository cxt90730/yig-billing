...
val df4 = sql("select bucketname,storageclass,sum((size + 65536 - 1) / 65536 * 65536) as usage from objects where storageclass=3 group by bucketname,storageclass")
val df5 = sql("select m.bucketname,m.storageclass, sum((mp.size + 65536 - 1) / 65536 * 65536) as usage from multipartpart mp left join multiparts m on m.bucketname = mp.bucketname and m.uploadtime=mp.uploadtime group by m.bucketname,m.storageclass").where("storageclass=3")
